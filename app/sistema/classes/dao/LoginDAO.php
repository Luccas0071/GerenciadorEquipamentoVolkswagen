...
include_once '../classes/facade/LoginFacade.php';
include_once '../classes/form/LoginForm.php';
include_once '../classes/model/Login.php';

class LoginDAO extends DAOFactory{
    
    public function obterDadosLogin($objLogin){
		...
		$sql .= " 	usu_fone, ";
		$sql .= " 	usu_email ";
		$sql .= " FROM gerenciador.tb_usuario ";
		$sql .= " WHERE usu_login = :loginUsuario ";
		$sql .= " AND usu_senha = :senhaUsuario ";

        $query = parent::$connection->pdo->prepare($sql);

        $query->bindParam(':loginUsuario', 		$objLogin->getLogin(), 		PDO::PARAM_STR);
		$query->bindParam(':senhaUsuario', 		$objLogin->getSenha(), 		PDO::PARAM_STR);
        ...
        if ($query->execute()) {
			...
			$objLogin->setCodigo($rs['usu_codigo']);
			$objLogin->setLogin($rs['usu_login']);
			$objLogin->setSenha($rs['usu_senha']);
			$objLogin->setNome($rs['usu_nome']);
			$objLogin->setDddFone($rs['usu_dddfone']);
			$objLogin->setFone($rs['usu_fone']);
			$objLogin->setEmail($rs['usu_email']);

		} else {
			$collectionErro = $query->errorInfo();
			throw new Exception("LoginDAO->obterDadosLogin " . $collectionErro[2]);
		}
        return $objLogin;
    }
